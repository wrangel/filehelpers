...
object UseCaseFactory {
  ...
  private class RenameFilesWithCommonPrefix(
    ...
    def run(): Unit = {
      files.foreach { filePath: Path =>
        val parent: String = filePath.getParent.toString
        val fileName: String = filePath.getFileName.toString
        val newPath: Path = Paths.get(
          parent,
          additionalElement.get + Constants.PartitionString + fileName)
        info(s"Renaming $filePath to $newPath")
  ...
  private class RenameDiskStationConflictBackToOriginal(val directory: String)
      extends UseCase {
  ...
  private class moveToParentDirectory(val directory: String) extends UseCase {
    ...
    def run(): Unit = {
      FileUtilities
        ...
        .foreach { filePath: Path =>
          Files.move(filePath, Paths.get(directory, filePath.getFileName.toString))
        }
    }
  }
  ...
  private class FindDuplicateFiles(val directory: String) extends UseCase {
    ...
    def run(): Unit = {
        ...
        filePath: Path =>
          ...
          (
            ...
            Files.size(filePath), {
              ...
              val p =
                Process(s"""exiftool -DateTimeOriginal "$filePath"""".trim).!!
              Try {
                StringUtilities.convertStringToLong(
      ...
      val occurrences: Map[Path, Int] = basics
        .groupBy(_._1)
        .view
        .mapValues(_.size)
        .toMap
      // Collect duplicates and their characteristics
      val overview: MMap[Path, ListBuffer[(Path, Long, Long, Long)]] =
        MMap[Path, ListBuffer[(Path, Long, Long, Long)]]()
      basics.foreach {
        case (newPath: Path,
              filePath: Path,
              size: Long,
          ...
          if (occurrences(newPath) > 1) {
            if (overview.contains(newPath))
              ...
              overview.addOne(
                newPath -> ListBuffer(
                  (filePath, size, fileTimestamp, exifTimestamp)))
          }
      }
      // Triage files into one to be renamed, and all others to be deleted
      val triage
        : MMap[Path, Map[Boolean, ListBuffer[(Path, Long, Long, Long)]]] =
        overview.map {
          case (newPath: Path,
                characteristics: ListBuffer[(Path, Long, Long, Long)]) =>
            newPath ->
              characteristics.groupBy { element: (Path, Long, Long, Long) =>
                ...
                case Success(del: (Long, Long)) =>
                  ...
                  if (ok._1 >= del._2) {
                    ...
                    FileUtilities.deleteFiles(
                      element._2(false).map(_._1) ++ renamingCandidates.tail
                        .map(_._1))
                    FileUtilities.renameFile(renamingCandidates.head._1,
                                             element._1)
                  }
      ...
      files.foreach { filePath: Path =>
        ...
        val newPath: Path =
          if (filePath.toString.contains(element)) {
            val Seq(name: String, extension: String) =
              FileUtilities.splitExtension(filePath, isPathNeeded = false)
            Paths.get(
              filePath.getParent.toString,
              name.substring(name.lastIndexOf(element) + element.length,
  ...
  def apply(
    ...
    useCase match {
      ...
      case "deleteDuplicateSubstring" =>
        new DeleteDuplicateSubstring(directory, additionalElement)
      case "moveToParentDir" =>
        new moveToParentDirectory(directory)
    }
  }
